...
CREATE TABLE IF NOT EXISTS bq_transactions (
  ...
  transaction_details_id UUID NOT NULL REFERENCES bq_transaction_details(transaction_details_id) ON DELETE CASCADE,
  ...
  reconciled_by UUID REFERENCES bq_users(user_id),
...
CREATE TABLE IF NOT EXISTS bq_transaction_details (
